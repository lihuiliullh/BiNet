# path no split, no relational chain 
import torch
import torch
...
class RelationExtractor(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, relation_dim, num_entities, pretrained_node_embeddings, pretrained_relation_embeddings, 
        ...
        #self.position_embedding = torch.Tensor([[[[0], [1], [2], [3], [4], [5]]]]).to(self.device)
    ...
    def attention_path_decoder(self, hidden, cell_state, batch_size, hop_num, SOS=None, path=None, teacher_forcing_ratio=0.5):
        ...
        #decoder_celll = cell_state.unsqueeze(0)
    ...
    def beam_search(self, head_index, head_embedidng, path_embeddings, beam_width=3, use_sigmoid=True):
    ...
    def refinement(self, head_embedding, relation_embeddings, tail_embedding, question_embedding, topk):
        ...
        # #head_embedding.view(-1, self.relation_dim)
        ...
        # output, (hidden, cell) = self.relation_reasoning_gru(head_embedding, (decoder_hidden, decoder_celll))
        # decoder_hidden = hidden
        # decoder_celll = cell
        ...
        # # finally
        # output, (hidden, cell) = self.relation_reasoning_gru(tail_embedding, (decoder_hidden, decoder_celll))
        ...
        # final output to probability. 0 - 1
        scores = self.relation_reasoning_predictor(output)
    ...
    def forward(self, sentence, p_head, p_tail, question_len, p_path):
        ...
        kgc=True
        if kgc: # improve the KGC performance (KGC and KGQA achieve best performance in different epochs)
            ...
            # here are many different strategies.
            # keep top 1, or keep top k (beam search)
        ...
        refine = True
        relation_reasoning_loss = 0
        if refine: # refine is reranking 
            ################################################################################################
            # refinement, reranking answers find by KGQA
            TOPK = 5
            top_score, top_idx = torch.topk(kgqa_score, k=TOPK, largest=True, sorted=True) # entity answers to be reranked
    ...
    def get_score_ranked(self, head, sentence, sent_len):
        ...
        pathes_embeddings, path_prediction_list = self.attention_path_decoder(hidden_, cell_state_, head.size()[0], hop_num=3, SOS=None) # decode path
        
        
        most_likely_relation = []

        for i in range(len(path_prediction_list)):
            path_pred = path_prediction_list[i]
            # path_loss
            _, pred_indices = torch.topk(path_pred, k=1)
            most_likely_relation.append(pred_indices.item())

        # select top k from kgqa predict
        TOPK = 5
        top_score, top_idx = torch.topk(score, k=TOPK, largest=True, sorted=True)
        tail_embedding = self.pretrained_node_embeddings(top_idx)
        a_ = self.refinement(head, pathes_embeddings, tail_embedding, rel_embedding, TOPK) # rerank KGQA 
        relation_reasoning_score, relation_reasoning_idx = torch.topk(a_, k=2, largest=True, sorted=True) # get top2 of new id
        a_actual = torch.gather(top_idx, 1, relation_reasoning_idx) # new id to origin id 

        # answers of KGQA without rerank, (new score with rerank, results of rerank), predicted path
        return top2, (relation_reasoning_score, a_actual), most_likely_relation
