...
class RelationExtractor(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, relation_dim, num_entities, pretrained_node_embeddings, pretrained_relation_embeddings, 
        ...
        self.position_embedding = torch.Tensor([[[[0], [1], [2], [3], [4]]]]).to(self.device)
    ...
    def forward(self, sentence, p_head, p_tail, question_len, p_path):
        ...
        refine = False
        if p_path.size()[1] == 3 or p_path.size()[1] == 1:
            refine = True
        ...
        if refine:
            ...
            a_ = self.refinement(p_head, [rel_embedding], tail_embedding, rel_embedding, TOPK)
    ...
    def get_score_ranked(self, head, sentence, sent_len):
        ...
        pathes_embeddings, path_prediction_list = self.attention_path_decoder(hidden, cell_state, head.size()[0], hop_num=1, SOS=None)
        most_likely_relation = torch.topk(path_prediction_list[0], k=1)
        ...
        return top2, (relation_reasoning_score, a_actual), most_likely_relation[1].item()
