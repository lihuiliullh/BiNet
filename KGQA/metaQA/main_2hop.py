...
from model_2hop_half import RelationExtractor
    ...
    for i in tqdm(range(len(data))):
        try:
            ...
            if top_2_idx_reasoning[0] == head_idx:
                pred_ans = top_2_idx_reasoning[1]
            else:
                pred_ans = top_2_idx_reasoning[0]
            is_correct_reasoning = 0
            if pred_ans in ans:
                total_correct_reasoning += 1
                is_correct_reasoning = 1
            q_text = d[-1]
            answers_reasoning.append(q_text + '\t' + str(pred_ans) + '\t' + str(is_correct_reasoning))
    ...
    for epoch in range(nb_epochs):
        ...
        for phase in phases:
            if phase == 'train':
                ...
                model.eval()
                eps = 0.0001
                answers, score, answer_rs, score_rs = validate(model=model, data_path= valid_data_path, word2idx= word2ix, entity2idx= entity2idx, device=device, model_name=model_name)
                if score > best_score + eps:
                    best_score = score
                    no_update = 0
                    best_model = model.state_dict()
                    print(hops + " hop Validation accuracy increased from previous epoch", score)
                    _, test_score, answer_rs, score_rs = validate(model=model, data_path= test_data_path, word2idx= word2ix, entity2idx= entity2idx, device=device, model_name=model_name)
